...
import { DersKonu, DersKonuRequest } from '../models/ders-konu';
import { environment } from '../../../environments/environment';

@Injectable({
  providedIn: 'root'
})
export class DersKonuService {
  private http = inject(HttpClient);
  private apiUrl = `${environment.apiUrl}/derskonu`;

  /**
   * Get all topics for a course in proper order (sorted by bolumNumara, konuSiraNo)
   */
  getAllByDersIdOrdered(dersId: number): Observable<DersKonu[]> {
    return this.http.get<DersKonu[]>(`${this.apiUrl}/by-ders/${dersId}/ordered`);
  }

  /**
   * Get all topics for a course (unordered)
   */
  getAllByDersId(dersId: number): Observable<DersKonu[]> {
    return this.http.get<DersKonu[]>(`${this.apiUrl}/by-ders/${dersId}`);
  }

  /**
   * Create a new topic
   */
  create(request: DersKonuRequest): Observable<DersKonu> {
    return this.http.post<DersKonu>(this.apiUrl, request);
  }

  /**
   * Update an existing topic
   */
  update(id: number, request: DersKonuRequest): Observable<DersKonu> {
    return this.http.put<DersKonu>(`${this.apiUrl}/${id}`, request);
  }
  ...
  /**
   * Delete a section by dersId and bolumNumara
   */
  deleteSection(dersId: number, bolumNumara: number): Observable<void> {
