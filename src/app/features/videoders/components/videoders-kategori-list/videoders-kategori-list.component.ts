...
export class VideodersKategoriListComponent implements OnInit {
  ...
  loadKategoriOzet() {
    this.loading = true;
    this.videodersKategoriService.getKategoriOzetByDersId(this.dersId)
      .subscribe({
        next: (data) => {
  ...
  onDelete(item: DersKategori) {
    ...
    this.videodersKategoriService.deleteKategori(this.dersId, kategoriId)
      .subscribe({
        next: () => {
  ...
  openKategoriModal() {
    if (this.kategoriModalTemplate) {
      const modalRef = this.modalService.open(this.kategoriModalTemplate, { size: 'lg' });
    }
  }
  ...
  addSelectedKategoriler() {
    ...
    const requests = this.selectedKategoriler.map(kategoriId =>
      this.videodersKategoriService.addKategori(this.dersId, kategoriId)
    );
